...
import { createAdminBlogPostApi, ApiCustomError } from '@/lib/api';
import { ArrowLeft, Save, Eye, Edit3, Star, AlertCircle } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (apiErr.status === 409) {
        ...
        setError(apiErr.message || 'Failed to create blog post.');
      }
    } finally {
      setSubmitting(false);
    }
          ...
          <form onSubmit={handleSubmit} className="p-8 rounded-3xl bg-gray-950 border border-gray-800 space-y-6 text-xs sm:text-sm">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-1.5 md:col-span-2">
                ...
                <input
                  ...
                  placeholder="e.g. Building Scalable APIs with Express & TypeScript"
                  required
              ...
              <div className="space-y-1.5">
                <label className="block font-bold text-gray-300">URL Slug * (Must be unique)</label>
                <input
                  type="text"
                  value={slug}
                  onChange={(e) => setSlug(e.target.value)}
                  placeholder="building-scalable-apis-express-typescript"
                  required
            ...
            <div className="pt-4 flex items-center justify-end gap-3 border-t border-gray-800">
              ...
              >
                ...
                <span>{submitting ? 'Saving Post...' : 'Save Post'}</span>
              </button>
            </div>
          </form>
        )}
